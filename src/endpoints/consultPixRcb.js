...
const consultPixRcb = async(req, res)=>{
    var statusCode = 400
    try{
        ...
        const listPix = await reqGN.get(`/v2/pix?inicio=${convertInit}T${inicioHr}Z&fim=${convertEnd}T${fimHr}Z`)
        const result = listPix.data

        if(result.pix.length === 0){
            statusCode = 403
            throw new Error('Não hoveram recebimentos de pix nesse período')
        }
